...
extension Queue {
  class Processors {
    ...
    var configurations = [Job.TypeName: ProcessorConfiguration]()
    var active = [Job.TypeName: [Job.ID: AnyJobProcessor]]()
    ...
    /**
     ...
     the result
     - Returns: a `[Job.ID: AnyJobProcessor]` that only includes active processors
     and excludes processors for any job whose id is in the `excludedIDs` list.
     */
    func activeProcessorsByID(excluding excludedIDs: [Job.ID]) -> [Job.ID: AnyJobProcessor] {
      return self.active.reduce(into: [Job.ID: AnyJobProcessor]()) { acc, kvp in
        kvp.value.filter {
    ...
    /**
     ...
     - Parameter processorsToRemoveByJob.ID: the job IDs whose associated processors
     should be removed from the active processors collection.
     */
    func remove(processors processorsToRemoveByJobID: [Job.ID]) {
      self.active = self.active.reduce(into: [Job.TypeName: [Job.ID: AnyJobProcessor]]()) { acc, kvp in
     ...
     */
    func isProcessing(job: Job) -> Bool {
     ...
     */
    @discardableResult
    func activeProcessor(for job: Job) -> AnyJobProcessor? {
      var nextProcessors = self.active[job.type, default: [Job.ID: AnyJobProcessor]()]
